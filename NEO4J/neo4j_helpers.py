...
if __name__ == '__main__':
    ...
    config.read('neo4j_config.ini')
    g = Neo4jGraph(showstatus=False, **config['DEFAULT'])

    g.deleteAllNodes(DEBUG=True)
    assert g.nodesNb == 0

    # duplicate node test
    imagery1 = createNode(g, 'Imagery', DEBUG=True, **{'detail': 'DARK CLOUD'})
    imagery2 = createNode(g, 'Imagery', DEBUG=True, **{'detail': 'DARK CLOUD'})
    assert g.nodesNb == 1

#%%
    # g.setGlobalConstraint(ADD=False)
    # g.setGlobalConstraint()

    # the user_id is already in the labels, but here I
    # show we can modify the dataclass at creation
    user1 = NewUser(properties={'user_id': '1234',
                                'display_name': 'JP',
                                'password_hash': 123})

    n1 = g.mergeNode(user1, DEBUG=True)  # displays node ID
    g.showNode(n1)
    assert g.nodesNb == 2

    print(user1.labels, user1.properties)

#%%

    g.mergeNode(user1, DEBUG=True)  # triggers duplicate detection
    assert g.nodesNb == 2

    question1 = Question(properties={'text': 'What kind of cloud is it?',
                                     'timestamp': Now()})
    # we can modify the constraints if we wish
    question1.labels_constraints = ('JP', 'Question')
    question1.properties_constraints = ('text',)

    question2 = Question(properties={'text': 'What type of cloud is it?'})
    question2.labels_constraints = ('JP', 'Question')
    question2.properties_constraints = ('text',)

    n2 = g.mergeNode(question1, DEBUG=True)
    g.mergeNode(question1, DEBUG=True)  # triggers duplicate detection
    g.mergeNode(question2, DEBUG=True)  # not a duplicate
    assert g.nodesNb == 4

    g.createRelation(n1, n2, Utters, DEBUG=True)

    # outputs
    # New node ('JP',) created with ID = 341
    # Node 341 has labels: :JP
    # Node 341 has properties:
    # 			 user_id:1234
    # 			 password_hash:123
    # 			 display_name:JP

    # Node ('JP',) already exists with ID = 341
    # New node ('JP', 'Question') created with ID = 342
    # Node ('JP', 'Question') already exists with ID = 342
    # New node ('JP', 'Question') created with ID = 343
    # Relationship 'UTTERS' created between nodes 341 and node 342

    transcripts = Transcripts(properties={'text': '',
                                          'timestamp': Now()})
    transcripts.properties_constraints = tuple()

    tr = g.mergeNode(transcripts, DEBUG=True )
    g.showNode(tr)
    assert g.nodesNb == 5

    # New node ('JP', 'Transcript') created with ID = 428
    # Node 428 has labels: :JP:Transcript
    # Node 428 has properties:
    # 			 text:
    # 			 timestamp:2022-09-30 00:48:34.326442

    # we can replace a property
    g.editProperty(tr,
                   text=["1:"+"I feel like I'm under a dark cloud"+" - "+Now()]
                   )
    g.showNode(tr)

    # Node None has labels: :JP:Transcript
    # Node None has properties:
    # 			 text:["1:I feel like I'm under a dark cloud- 2022-10-07 00:09:02.523395"]
    # 			 timestamp:2022-10-07 00:09:02.502252

    # we can add new text into a property by specifying append=True
    g.editProperty(tr, append=True,
                   text=["2:"+"I can't see the tree from the forest"+" - "+Now(),
                          "3:"+"I see the light at the end of the tunnel"+" - "+Now()]
                  )
    g.showNode(tr)
    ...
    # Node None has properties:
    # 			 text:["1:I feel like I'm under a dark cloud - 2022-10-07 00:09:02.523395",
    # 			 "2:I can't see the tree from the forest - 2022-10-07 00:09:02.611538",
    # 			 '3:I see the light at the end of the tunnel - 2022-10-07 00:09:02.611561']
    # 			 timestamp:2022-10-07 00:09:02.502252

    # we can add a string alone, without [] or ()
    g.editProperty(tr, append=True,
                   text="4:" + "I am on fire" + " - " + Now()
                   )
    g.showNode(tr)
    # Node 56 has labels: :JP:Transcripts
    # Node 56 has properties:
    # 			 text:["1:I feel like I'm under a dark cloud - 2022-10-07 00:12:26.053367", "2:I can't see the tree from the forest - 2022-10-07 00:12:26.064713", '3:I see the light at the end of the tunnel - 2022-10-07 00:12:26.064732', '4:I am on fire - 2022-10-07 00:12:26.095069']
    # 			 timestamp:2022-10-07 00:12:26.035251

    print(bcolors.OKBLUE + "TEST ENDED SUCCESSFULLY" + bcolors.END)